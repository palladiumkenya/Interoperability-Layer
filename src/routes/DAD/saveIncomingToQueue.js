...
import {
  ...
  getMsgRecepients,
  getCCCNumber
} from './message.manipulation'
import { updateNumericStats, updateMsgStats } from '../../lib/stats.logic'
  ...
  try {
    ...
    if (identifier && identifier.ID) {
      ...
      response = JSON.stringify({
        msg: 'successfully received by the Interoperability Layer (IL)'
      })
    } else {
      response = JSON.stringify({
        msg:
          'No CCC Number specified! This message will not be shared with the systems as it does not follow the standard!'
      })
    }
  } catch (error) {
...
export const saveJSONToQueue = async payload => {
  ...
  let subscriberList = msgRecepients.length
    ? `Subscribers include ${msgRecepients.map(rec => rec.name).join(', ')}`
    : `No subscribers listed for this message yet!`
  if (CCCNumber) {
    receivedMsgLog = `Received ${messageType.verboseName.replace(
      /_/g,
      ' '
    )} message (${CCCNumber.IDENTIFIER_TYPE} : ${
      CCCNumber.ID
    }) from ${sendingApplication}. ${subscriberList}`
  } else {
    const randomIdentifier = getRandomIdentifier(payload)
    receivedMsgLog = `Received ${messageType.verboseName.replace(
      /_/g,
      ' '
    )} message (${randomIdentifier.IDENTIFIER_TYPE} : ${
      randomIdentifier.ID
    }) from ${sendingApplication} with no CCC number! . ${subscriberList}`
  }
...
export const saveXMLToQueue = async payload => {
  ...
  try {
    const messageType = await getMessageTypeObj(messageTypeName)
    const entities = await getSubscribedEntities(messageType)
    const receivedMsgLog = `Received ADX message successfully. Subscribers include ${entities
